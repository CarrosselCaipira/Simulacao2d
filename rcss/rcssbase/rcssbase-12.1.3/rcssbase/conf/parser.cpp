...
static
void
setPath( const char * begin,
         const char * end )
{
    std::string spath = cleanString( begin, end );
    ...
    lib::Loader::setPath( path );
}

static
void
addPath( const char * begin,
         const char * end )
{
    std::string spath = cleanString( begin, end );
    boost::filesystem::path path;
...
void
Parser::load( const char * begin,
              const char * end )
{
    lib::Loader loader;
    std::string libname = cleanString( begin, end );
    boost::filesystem::path path;
    try
    {
    ...
    catch ( ... )
    {
        ...
        {
            m_builder.loadFailed( libname,
                                  e.what(),
                                  lib::Loader::listAvailableModules(),
                                  m_stack.front().m_name,
                                  m_stack.front().m_lineno );
        }
    }
    ...
    {
        m_builder.manageModule( loader );
        Builder::Creator c;
        if ( Builder::factory().getCreator( c,
                                            loader.strippedName().c_str() ) )
        {
            m_builder.manageChild( c( &m_builder ) );
        }
    }
}
...
Parser::boostParse()
{
    ...
    boost::spirit::rule<> flag_p = ( boost::spirit::as_lower_d[ "help" ]
                                     ...
                                     | boost::spirit::as_lower_d[ "load" ]
                                     | boost::spirit::as_lower_d[ "setpath" ]
                                     | boost::spirit::as_lower_d[ "addpath" ]
                                     );
    ...
    boost::spirit::rule<> loadrule_p
        = ( boost::spirit::as_lower_d[ "load" ]
            >> ignore_p
            >> expect_assign( assign_p )
            >> ignore_p
            >> expect_string( string_p[ boost::bind( &Parser::load,
                                                     this,
                                                     _1, _2 ) ] )
            );


    boost::spirit::rule<> pathrule_p = ( ( boost::spirit::as_lower_d[ "setpath" ]
                                           >> ignore_p
                                           >> expect_assign( assign_p )
                                           >> ignore_p
                                           >> expect_string( string_p[ &setPath ] )
                                           )
                                         | ( boost::spirit::as_lower_d[ "addpath" ]
                                             >> ignore_p
                                             >> expect_assign( assign_p )
                                             >> ignore_p
                                             >> expect_string( string_p[ &addPath ] )
                                             )
                                         );
